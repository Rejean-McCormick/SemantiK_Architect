...
def generate_grammars():
    ...
    # We maintain the skip list for truly incompatible languages
    ...
    for wiki_code, rgl_code in module_map.items():
        ...
        # We assume if Cat/Noun exist, the main Grammar module likely exists too.
        ...
        # THE FIX: Import the full 'GrammarX' to satisfy SyntaxX dependencies.
        # We also need ParadigmsX for the lexicon.
        imports = [
            f"Grammar{rgl_code}",
            f"Paradigms{rgl_code}"
        ]

        # We now safely use standard Syntax constructors (mkNP, mkCN)
        content = f"""
concrete {wiki_mod} of Wiki = {", ".join(imports)} ** open Syntax{rgl_code}, (P = {imports[-1]}) in {{
  ...
  lin
    ...
    -- Lexicon
    -- Now mkNP and mkCN will work because types match Grammar{rgl_code}
...
"""
    ...
    print(f"\nSuccessfully generated {len(generated_langs)} language files (Standard RGL Mode).")
