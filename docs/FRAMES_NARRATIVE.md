...
* `CareerSeasonCampaignFrame`
  Specialized timeline for one coherent trajectory (career, sports season, political campaign).
...
3. **Stable `frame_type` values**
   ...
   * Every frame declares a stable `frame_type` string (`"timeline"`, `"career-season"`, etc.) so that the planner and engines can dispatch on type the same way they do for `BioFrame` and `EventFrame`.
...
* **Subject reference**
  ...
  * `subject_id: str | None` (e.g. Wikidata QID or local ID).
  * Optionally `subject: Entity | None` for inline usage if needed.

* **Overall time span**

  * `span: TimeSpan | None` – the overall window for the narrative.
...
* **Item lists**
  ...
  class TimelineItem:
      ...
      phase_id: str | None = None       # grouping into phases, if any
      salience: int | None = None       # 0 = highest priority
  ```
...
class TimelineFrame(Frame):
    frame_type: str = "timeline"

    subject_id: str | None = None    # e.g. main entity ID
    span: TimeSpan | None = None     # overall period, optional
    items: list[TimelineItem] = field(default_factory=list)

    # Optional hints
    granularity: str | None = None   # "year", "month", "day", "decade", ...
    ordering: str = "chronological"  # or "reverse-chronological"
...
class TimelineItem:
    ...
    salience: int | None = None      # lower = earlier in narrative
    phase_id: str | None = None      # to group into phases/periods
```

**Generation behavior (informal)**

* Planner sorts `items` by `event.time` (or explicit order / `salience`).
* Groups into phases if `phase_id` is used, optionally mapping phases to headings or lead sentences.
...
```python
@dataclass
class CareerSeasonCampaignFrame(Frame):
    frame_type: str = "career-season"
...
### 4.5 `StructureOrganizationFrame`
...
class StructureOrganizationFrame(Frame):
    frame_type: str = "structure-organization"
...
### 4.6 `ComparisonSetContrastFrame`
...
class ListEnumerationFrame(Frame):
    frame_type: str = "list-enumeration"
...
1. **Input**
   ...
   * A frontend client or AW bridge passes a `NarrativeFrame` (one of the types above) to `generate` / `NLGSession.generate`.
...
* `EventFrame` and the narrative frames specified here are, for now, **API- and schema-level specifications**: they define the data model that future normalizers, planners, and engines will support.
* Implementations can proceed incrementally:

  * Start with `TimelineFrame` and `ListEnumerationFrame` (they reuse `Event` and simple entity lists).
  * Add domain-specific logic for `CareerSeasonCampaignFrame` and `ReceptionImpactFrame`.
