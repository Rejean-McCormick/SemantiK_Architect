...
function normalizeCli(cli: unknown): string[] {
  if (!cli) return [];
  ...
  if (typeof cli === "string") {
    // Allow newline-separated commands; single-line stays as one entry.
    return cli
      .split("\n")
      .map((s) => s.trim())
      .filter(Boolean);
                ...
                .map(([groupName, groupItems]) => (
                  <div key={groupName} className="space-y-2 pl-2">
                    ...
                    <div className="space-y-2">
                      {groupItems.map((it) => {
                        const cliLines = normalizeCli((it as any).cli);
                        const uiSteps = it.uiSteps ?? [];
                        const notes = it.notes ?? [];
                        const repoHref = repoFileUrl(repoUrl, it.path) || null;
                                    ...
                                    <div className="mt-3">
                                      ...
                                      {it.commandPreview ? (
                                        <div className="mt-2 text-[11px] text-slate-500">
                                          Command preview:{" "}
                                          <span className="font-mono">{it.commandPreview}</span>
                                        </div>
                                      ) : null}
                                    </div>
                                  ) : (
