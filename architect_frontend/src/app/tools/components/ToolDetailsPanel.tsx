...
import { Copy, ExternalLink, Info, Loader2, Play, X } from "lucide-react";
...
export function ToolDetailsPanel(props: ToolDetailsPanelProps) {
  ...
  // Compute everything with null-safe guards, then early-return after hooks.
  const s = selected;

  const wiredToolId = s?.wiredToolId || null;
  ...
  const repoHref = useMemo(() => {
    if (!s) return "";
    return repoFileUrl(repoUrl, s.path);
  }, [repoUrl, s?.path]);

  const cliLines = useMemo(() => {
    const raw = (s?.cli || "").trim();
    if (!raw) return [];
    return raw
      .split("\n")
      .map((line) => line.trim())
  ...
  const fullPreview = useMemo(() => {
    const base = (s?.commandPreview || s?.cli || "").trim();
  ...
  }, [s?.commandPreview, s?.cli, extraArgs]);
  ...
  const handleRun = useCallback(() => {
    ...
    if (s.risk === "heavy") {
      const ok = window.confirm(
        "This tool is marked HEAVY and may take a while or use significant resources. Run it?"
      );
      if (!ok) return;
    }
  ...
  // Early return AFTER hooks (fixes react-hooks/rules-of-hooks)
  if (!s) {
    return (
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2 text-xs text-slate-500">
                  <span className="font-medium text-slate-600">{s.category}</span>
                  <span>/</span>
                  <span>{s.group}</span>
                  <span>•</span>
                  <span className="font-mono bg-slate-100 px-1 rounded">{s.kind}</span>
                </div>
              </div>
              <div className="flex flex-col items-end gap-1.5 shrink-0">
                ...
                <WiringBadge wired={Boolean(s.wiredToolId)} hidden={Boolean(s.hidden)} />
              </div>
            </div>
              ...
              {!isWired ? (
                <div className="text-sm text-slate-500 italic">
                  ...
                  <div className="mt-1 text-xs">
                    Guessed ID: <span className="font-mono">{s.toolIdGuess}</span>
                  </div>
                </div>
              ) : (
                <div className="space-y-3">
                  ...
                  <div className="space-y-1">
                    ...
                    {Array.isArray(parsedArgs) && parsedArgs.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 pt-1">
                        {parsedArgs.slice(0, 12).map((a: string, i: number) => (
                          <span
                            key={`${String(a)}-${i}`}
                  ...
                  <div className="pt-1 flex flex-wrap gap-2">
                    <Button
                      onClick={handleRun}
                      disabled={isBusy}
                      variant={s.risk === "heavy" ? "destructive" : "default"}
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-3 space-y-2">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  size="sm"
                  className="h-7 text-xs px-2 text-slate-500 hover:text-slate-900"
                  onClick={() => copyToClipboard(s.path)}
                >
          ...
          {cliLines.length > 0 && (
            <div className="space-y-2">
              ...
              <div className="rounded-md border border-slate-200 bg-slate-50 divide-y divide-slate-200">
                {cliLines.map((cmd: string, i: number) => (
          ...
          {s.uiSteps && s.uiSteps.length > 0 && (
            <div className="space-y-2">
              ...
              <ul className="list-disc list-inside space-y-1 text-xs text-slate-600 bg-white p-2 rounded-md border border-slate-200">
                {s.uiSteps.map((step: string, i: number) => (
