 ...
 *
 * Assumptions:
 * - API functions are exposed via `architectApi` in ../lib/api, e.g.:
 *     - architectApi.fetchFramesMetadata()
 *     - architectApi.generateFrame(slug: string, payload: Record<string, unknown>)
 *     - architectApi.getSuggestions(payload: { query: string; [key: string]: unknown })
 ...
 * update the wrappers below to match.
 */
...
// Wrap the actual API object in `any` so tests compile even if typings
// in ../lib/api don’t yet expose these helpers explicitly.
const apiAny = architectApi as any;
