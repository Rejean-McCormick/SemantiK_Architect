...
def render_romance_bio(name, gender, prof_lemma, nat_lemma, lang_config):
    ...
    def get_indefinite_article(next_word, target_gender):
        ...
        if impure_triggers:
            ...
            starts_match = any(
                next_word.startswith(t) for t in impure_triggers if t != "s_consonant"
            )
    ...
    sentence = sentence.replace(
        "{sep}", sep
    )  # The JSON specifically separates article/sep
...
if __name__ == "__main__":
    ...
    mock_it_config = {
        ...
        "morphology": {
            "suffixes": [{"ends_with": "tore", "replace_with": "trice"}],
            "irregulars": {},
        },
        "phonetics": {"impure_triggers": ["z", "gn"]},
