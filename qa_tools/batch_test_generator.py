...
NAMES = {
    "Male": ["Alessandro", "Pierre", "Juan", "João", "Mateo", "Sigmund", "Hans", "Dante", "Mateusz", "Ivan"],
    "Female": ["Maria", "Sophie", "Elena", "Ana", "Ioana", "Frida", "Marie", "Julia", "Sarah", "Alda"]
}
...
VOCAB = {
    "professions": {
        "Physicist": {"it": "fisico", "es": "físico", "fr": "physicien", "pt": "físico", "ro": "fizician", "de": "Physiker", "en": "physicist", "ru": "физик", "hi": "bhautik_vigyani"},
        "Chemist": {"it": "chimico", "es": "químico", "fr": "chimiste", "pt": "químico", "ro": "chimist", "de": "Chemiker", "en": "chemist", "ru": "химик", "hi": "rasayan_shastri"},
        "Writer": {"it": "scrittore", "es": "escritor", "fr": "écrivain", "pt": "escritor", "ro": "scriitor", "de": "Schriftsteller", "en": "writer", "ru": "писатель", "hi": "lekhak"},
        "Doctor": {"it": "medico", "es": "doctor", "fr": "docteur", "pt": "doutor", "ro": "doctor", "de": "Arzt", "en": "doctor", "ru": "доктор", "hi": "tabib"},
        "Teacher": {"it": "insegnante", "es": "profesor", "fr": "enseignant", "pt": "professor", "ro": "profesor", "de": "Lehrer", "en": "teacher", "ru": "учитель", "hi": "adhyapak"},
        "Politician": {"it": "politico", "es": "político", "fr": "politicien", "pt": "político", "ro": "politician", "de": "Politiker", "en": "politician", "ru": "политик", "hi": "neta"},
    },
    "nationalities": {
        "Italian": {"it": "italiano", "es": "italiano", "fr": "italien", "pt": "italiano", "ro": "italian", "de": "italienisch", "en": "italian", "ru": "итальянский", "hi": "bharatiya"},
        "French": {"it": "francese", "es": "francés", "fr": "français", "pt": "francês", "ro": "francez", "de": "französisch", "en": "french", "ru": "французский", "hi": "fransisi"},
        "German": {"it": "tedesco", "es": "alemán", "fr": "allemand", "pt": "alemão", "ro": "german", "de": "deutsch", "en": "german", "ru": "немецкий", "hi": "almani"},
        "Polish": {"it": "polacco", "es": "polaco", "fr": "polonais", "pt": "polonês", "ro": "polonez", "de": "polnisch", "en": "polish", "ru": "польский", "hi": "polish"},
        "American": {"it": "americano", "es": "americano", "fr": "américain", "pt": "americano", "ro": "american", "de": "amerikanisch", "en": "american", "ru": "американский", "hi": "ameriki"},
        "Russian": {"it": "russo", "es": "ruso", "fr": "russe", "pt": "russo", "ro": "rus", "de": "russisch", "en": "russian", "ru": "русский", "hi": "russian"},
    }
}
...
def feminize(word, lang):
    ...
    if lang in ['de']:
        if word.endswith('er') and word != 'Lehrer': return word[:-2] + 'erin'
        if word == 'Lehrer': return 'Lehrerin'
        if word == 'Arzt': return 'Ärztin'
        if word.endswith('e'): return word + 'rin'
        return word + 'in' # Generic
    
    if lang in ['ru', 'pl']: # Slavic Nouns
        if word.endswith('тель'): return word[:-4] + 'тельница' # Russian -tel
        if word == 'писатель': return 'писательница'
        if word == 'учитель': return 'учительница'
        if word == 'химик': return 'химичка'
        if word == 'доктор': return 'докторша'
        if word == 'физик': return 'физичка'
        if word.endswith('ик'): return word + 'иня'

    # Indo-Aryan Noun Feminization
    if lang in ['hi']:
        if word.endswith('ak'): return word[:-2] + 'ika' # lekhak -> lekhika
        if word.endswith('i') and word != 'neta': return word + 'n'
        if word == 'neta': return 'netri'
        if word == 'adhyapak': return 'adhyapika'
    ...
    if lang in ['fr', 'it', 'es', 'pt', 'ro']:
        ...
        if word.endswith('ês'): return word[:-2] + 'esa'
        if word.endswith('ain'): return word + 'e'
        if word.endswith('és'): return word + 'a'
        if word.endswith('or'): return word + 'a'
        if word.endswith('er'): return word + 'a'
        if word.endswith('al'): return word + 'a'
...
def inflect_case_and_adjective(word, gender, case, lang):
    ...
    if lang == 'de':
        ...
        if case == 'nominative':
            if gender == 'male': return word + 'er'
            if gender == 'female': return word + 'e'
            return word + 'es'
    ...
    if lang == 'ru' and case == 'instrumental':
        if gender == 'female': 
            if word.endswith('а') or word.endswith('я'): return word[:-1] + 'ой' # Nouns
            if word.endswith('ий') or word.endswith('ый'): return word[:-2] + 'ой' # Adjs
            if word.endswith('й'): return word[:-1] + 'ей'
            return word + 'ью' # Soft sign nouns
        if gender == 'male':
            if word.endswith('ик'): return word + 'ом'
            if word.endswith('ь'): return word[:-1] + 'ем'
            if word.endswith('а') or word.endswith('о'): return word + 'м'
            return word + 'ом'
...
def generate_sentence(name, gender, prof, nat, lang):
    ...
    if is_slavic:
        final_prof = inflect_case_and_adjective(real_prof_nom, gender, target_case, lang)
    ...
    if lang in ['it', 'es', 'fr', 'pt', 'en', 'de', 'sv']:
        ...
        if lang == 'it' and (final_prof.startswith('s') and final_prof[1] not in 'aeiou') or final_prof.startswith(('z', 'a', 'e', 'i', 'o', 'u')):
             art_str = 'uno ' if gender == 'Male' and final_prof.startswith(('s', 'z', 'gn')) else "un'" if final_prof.startswith(('a', 'e', 'i', 'o', 'u')) else 'un '
             sep = "" if art_str.endswith("'") else " "
        else:
            # Simple a/an for English
            if lang == 'en' and final_nat.lower().startswith('a'): art_str = 'an'
            sep = " "
