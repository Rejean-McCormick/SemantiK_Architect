...
def run_universal_tests():
    ...
    for filename in files:
        ...
        with open(filepath, 'r', encoding='utf-8') as f:
            ...
            prof_col = next((c for c in reader.fieldnames if c.startswith('Profession_Lemma')), None)
            nat_col = next((c for c in reader.fieldnames if c.startswith('Nationality_Lemma')), None)
            ...
            for row in reader:
                ...
                # Inputs
                name = row['Name']
                gender = row['Gender'] # or 'Gender (Male/Female)'
                if 'Gender' not in row and 'Gender (Male/Female)' in row:
                ...
                try:
                    ...
                    actual = router.render_biography(name, gender, prof_input, nat_input, lang_code)
                    ...
                    if actual == expected:
                        ...
                        print(f"      Input:    {name} ({gender}) | {prof_input} | {nat_input}")
        ...
        if file_passed + file_failed > 0:
            ...
            print(f"   📊 Result: {file_passed} Passed, {file_failed} Failed ({rate:.1f}%)")
        else:
